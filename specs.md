...
- **Camera Only / BGD is a status with no stored form (issue #48)** — every
  ...
  *stored* value would therefore show a boom-gate-down station as **Open** on
  ...
  - **Read:** builds that know the status derive it from the shared
    ...
    **It is a status there, not an activity (issue #52):** new builds do not
    list these reports under a card's *Recent reports* — every press of the
    blue button used to add a row (five taps, five "Camera Only" rows) while
    Open, Blitz and Closed only ever light their button; the button, the badge
    and "reported Xm ago" already say it. The one exception is a report that
    carries a note: only an old build's dialog can write that, and a driver's
    own words are information the status alone doesn't hold
    (`recentActivityReports`). For the same reason BGD / Camera Only are not
    offered in the *Report activity* dialog. Old builds cannot know any of
    this and keep listing one row per press — the only way they can see it.
...
| Alert beep audible over music (issue #18) | ✅ Done — `alertAudioContext()` in `alert_player.dart`: Android alarm stream; iOS `playback` category (ignores silent switch); web unaffected |
...
| Sites listed A–Z within each state (issue #36) | ✅ Done — `sortByName` in `services/site_stats.dart` (case-insensitive, ties on suburb then id) orders the state-detail list, which previously inherited the repository's newest-first order; applied after the search filter and before `pinSiteFirst`, so the tapped site still pins to the top (issue #10). Home's Recently Active (time-ordered) and Nearby (distance-ordered) are unchanged |
