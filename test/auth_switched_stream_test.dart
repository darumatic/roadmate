...
void main() {
  test('a null auth identity emits the signed-out value, no listener', () async {
  ...
  test(
    'signing out after signing in emits the signed-out value '
    '(the old asyncExpand shape queued it forever)',
    () async {
      final auth = StreamController<String?>();
      final emitted = <String>[];
      final sub = authSwitchedStream<String, String>(
        ...
        sourceOf: (uid) =>
            Stream.value('data:$uid').followedBy(StreamController<String>().stream),
        signedOutValue: 'signed-out',
      ).listen(emitted.add);
  ...
  test(
    'an account switch moves the listener to the new identity '
    '(favourites/stats used to stay frozen on the old account)',
    () async {
      final auth = StreamController<String?>();
      final emitted = <String>[];
      final sub = authSwitchedStream<String, String>(
  ...
  test('a repeat of the same identity does not churn a healthy listener',
      () async {
    final auth = StreamController<String?>();
    var opens = 0;
    final sub = authSwitchedStream<String, String>(
  ...
  test('onSwitch sees every auth emission before anything is emitted',
      () async {
    final auth = StreamController<String?>();
    final log = <String>[];
    final sub = authSwitchedStream<String, String>(
