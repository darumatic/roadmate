...
void main() {
  ...
  test('a dead doc listener is re-opened and recovers the profile '
      '(0.1.74: it stayed dead, so every post re-prompted)', () async {
    final auth = StreamController<ProfileAuthUser?>();
    var opens = 0;
    final emitted = <UserProfile?>[];
    final sub = profileStream(
      ...
      profileDocOf: (u) {
        ...
        if (opens == 1) {
          ...
          return Stream<UserProfile?>.error(StateError('permission denied'));
        }
        return Stream.value(
  ...
  test('a committed claim reaches the stream even with the doc listener dead '
    ...
    final sub = profileStream(
      ...
      profileDocOf: (u) => Stream.error(StateError('permission denied')),
  ...
  test(
    'a stale doc snapshot without a name never undoes a committed claim',
    () async {
      final auth = StreamController<ProfileAuthUser?>();
      final doc = StreamController<UserProfile?>();
      final claims = StreamController<ClaimedName>();
  ...
  test(
    'a token refresh (same identity) does not churn a healthy listener',
    () async {
      final auth = StreamController<ProfileAuthUser?>();
      var opens = 0;
      final sub = profileStream(
